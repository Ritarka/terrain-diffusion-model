...
def training_loop(train_dataloader, opts):
    ...
    for _ in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            D_real_loss = criterion(D(real_images), utils.to_var(torch.ones(len(batch))))

            # 2. Sample noise
            noise = sample_noise(len(batch), opts.noise_size)
            
            # 3. Generate fake images from the noise
            fake_images = G(noise)

            # 4. Compute the discriminator loss on the fake images
            D_fake_loss = criterion(D(fake_images), utils.to_var(torch.zeros(len(batch))))
            D_total_loss = (D_fake_loss + D_real_loss) * 0.5
            ...
            # G_loss = criterion(D(fake_images), utils.to_var(torch.ones(len(batch))))
            G_loss = torch.mean((D(fake_images) - 1)**2) * 0.5
            ...
            if (iteration > 6000 and iteration % 10 == 0) or iteration % opts.sample_every == 0:
